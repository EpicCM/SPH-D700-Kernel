...
//SecFeature shiks_DK25 start
#if defined(DTUN_DEVICE_FILTERING) && (DTUN_DEVICE_FILTERING == TRUE)
#define DEVICE_HEADSET  0
#define DEVICE_PC       1
#define DEVICE_PHONE    2
#define DEVICE_ALL      3

static uint32_t cod_filter = DEVICE_ALL;
#endif
//SecFeature shiks_DK25 end
...
//SecFeature shiks_DK25 start
#if defined(DTUN_DEVICE_FILTERING) && (DTUN_DEVICE_FILTERING == TRUE)
int dtunops_start_discovery_by_cod (int index, gboolean periodic, int cod)
{
    cod_filter = cod;
    dtun_client_call_id_only(DTUN_METHOD_DM_START_DISCOVERY);
    return 0;
...
int dtunops_set_security (int index, uint32_t mode)
{
    tDTUN_DEVICE_METHOD method;

    method.set_security.hdr.id = DTUN_METHOD_DM_SET_SECURITY;
    method.set_security.hdr.len
        = sizeof(tDTUN_METHOD_DM_SET_SECURITY) - sizeof(tDTUN_HDR);
    method.set_security.mode = mode;
...
//SecFeature shiks_DK25 start
#if defined(DTUN_DEVICE_FILTERING) && (DTUN_DEVICE_FILTERING == TRUE)
	.start_discovery_by_cod = dtunops_start_discovery_by_cod,
#endif
//SecFeature shiks_DK25 end
	...
	.set_security = dtunops_set_security,
};
...
void dtun_dm_sig_rmt_name(tDTUN_DEVICE_SIGNAL *msg)
{
    ...
    if (read_device_name(src, dst, old_name) == 0 && strcmp(name, old_name) == 0) {
        return;
    }
...
#define COD_MAJOR_AUDIO_VIDEO  0x0400
...
//SecFeature shiks_DK25 start
#if defined(DTUN_DEVICE_FILTERING) && (DTUN_DEVICE_FILTERING == TRUE)
    if (cod_filter == DEVICE_HEADSET) {
        major = msg->device_found.info.cod & COD_MAJOR_MASK;
        if (major != COD_MAJOR_AUDIO_VIDEO && major != COD_MAJOR_WEARABLE) {
            info("\t*** Device [%s] class major is NOT headset - skip it.\n\n", bdaddr_str);
            return;
        }
    }
    else if (cod_filter == DEVICE_PC) {
        if ((msg->device_found.info.cod & COD_MAJOR_MASK) != COD_MAJOR_COMPUTER) {
            info("\t*** Device [%s] class major is NOT computer - skip it.\n\n", bdaddr_str);
            return;
        }
    }
    else if (cod_filter == DEVICE_PHONE) {
        if ((msg->device_found.info.cod & COD_MAJOR_MASK) != COD_MAJOR_PHONE) {
            info("\t*** Device [%s] class major is NOT phone - skip it.\n\n", bdaddr_str);
            return;
        }
    }
    else {
...
void dtun_dm_sig_authorize_req(tDTUN_DEVICE_SIGNAL *msg)
{
	...
	else
	{
		...
		//+ thun.lim from Henry set a device name
		if ( strlen(msg->authorize_req.info.bd_name) > 0 )
		{
			info("\t*** calling hcid_dbus_remote_name with rmt_name=%s ***\n", msg->authorize_req.info.bd_name);
			write_device_name(&sba, &msg->authorize_req.info.bd_addr, msg->authorize_req.info.bd_name);
			hcid_dbus_remote_name(&sba, &msg->authorize_req.info.bd_addr, 0, msg->authorize_req.info.bd_name);
		}
...
*/
//SecFeature shiks_DK25 start
    while(retry-- && property_set(DTUN_PROPERTY_HCID_ACTIVE, "1") !=0)
	{
		LOGE("Property setting busy ?  %d : Retry, dtun_client_main, DTUN_PROPERTY_HCID_ACTIVE, 1", retry);
		usleep(100000); 		
	}
	if(retry == 0)
	{
		LOGE("property set failed");
	}
	else
	{
		LOGV("property set success");
	}
//SecFeature shiks_DK25 end
