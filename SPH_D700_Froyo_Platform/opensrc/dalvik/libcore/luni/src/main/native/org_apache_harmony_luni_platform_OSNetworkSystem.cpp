...
#define SPRINT_SLOT0 // SecFeature SLOT0
...
#ifdef SPRINT_SLOT0
static boolean check_slot0_groups(void)
{
    boolean ret = TRUE;
    gid_t list[20];
    int i, n;
    char type[PROPERTY_VALUE_MAX] = {NULL,};
    //LOGD("createSocketImpl ");
    property_get("net.connectivity.type", type, "");
    n = getgroups(20, list); //getgid();
    LOGD("check_slot0_groups num = %d", n);
    if(n != -1 && strcmp(type, "CDMA0") == 0)  // TODO: how to handle error when n is over 20.
    {
        for(i =0 ; i < n ; i++)
        {
            if(list[i] == AID_INET_SPRINT)
            {
                  LOGD("createSocketImpl gid = %d, type = %s", list[i], type);
            	break;
            }
        }
        if( i == n)
        {
            ret = FALSE;  //dose not have SLOT0 permission
        }
    }
    LOGD("check_slot0_groups ret = %d", ret);
    return ret;
}
#endif
...
static bool initCachedFields(JNIEnv* env) {
    ...
    struct classInfo {
        ...
        {&c->string_class, "java/lang/String"}  // [ Config.TelephonyFeature.CONFIG_MULTIPLE_PDP_FEATURE ]
    };
...
#ifdef SPRINT_SLOT0
	if(check_slot0_groups() == FALSE)
    {
        jniThrowSocketException(env, SOCKERR_WOULDBLOCK);
        errno = EAGAIN;
        return -1;
    }
#endif
...
static void osNetworkSystem_setSocketOptionImpl(JNIEnv* env, jclass clazz,
    ...
    } else if(env->IsInstanceOf(optVal, gCachedFields.string_class)){
        // [ Config.TelephonyFeature.CONFIG_MULTIPLE_PDP_FEATURE
    ...
    switch ((int) anOption & 0xffff) {
        ...
        case JAVASOCKOPT_SO_BINDTODEVICE: {
            ...
            lengthInterface = strlen(interfaceName);

            result = setsockopt(handle, SOL_SOCKET, SO_BINDTODEVICE, interfaceName,
                               strlen(interfaceName) + 1);
            if (result != 0) {		 		
                LOGE("Set BINDTODEVICE errono:  %s(%d)", strerror(errno), errno);
                throwSocketException(env, convertError(errno));
                return;
            }
            else {
                LOGE("Success: set BINDTODEVICE to %s", interfaceName);
            }
