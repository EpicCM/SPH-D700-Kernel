...
//Samsung SWP Bluetooth - For root permission
static void bt_deviceinfo_chown(void)
{
	char pathname[128] = {0, }, filename[128] = {0,};
	DIR *d, *dpath;
	struct dirent *de, *dpathe;

	d = opendir("/data/misc/bluetoothd");
	if(d == 0) return;

	while((de = readdir(d)) != 0){
		if(de->d_type != DT_DIR || de->d_name[0] == '.') {
			LOGE("this is not dir. type: %d, name: %s", de->d_type, de->d_name);
			continue;
		}
		...
		if(dpath == 0) return;
		while((dpathe = readdir(dpath)) != 0){
			if(dpathe->d_type != DT_REG || dpathe->d_name[0] == '.') {
				LOGE("this is not reg. type: %d, name: %s", de->d_type, de->d_name);
				continue;
			}
			sprintf(filename, "%s/%s", pathname, dpathe->d_name);
			if (chown(filename, AID_BLUETOOTH, AID_BLUETOOTH) < 0) {
				LOGE("Fail to change group ownership of %s to %d: %s",
				filename, AID_BLUETOOTH, strerror(errno));
			}
		}
		closedir(dpath);
	}
...
//End Samsung SWP Bluetooth

#ifdef BT_ALT_STACK
int main(int argc, char *argv[])
{

	bt_deviceinfo_chown(); //Samsung SWP Bluetooth - For root permission 
