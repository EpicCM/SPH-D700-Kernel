                                 ...
                                 void *data)
{
    ...
    err = adapter_ops->set_security(adapter->dev_id, mode);
    if (err < 0)
        return failed_strerror(msg, -err);
					...
					DBusMessage *msg, void *data)
{
	...
	if (g_str_equal("Name", property)) {
...
#ifdef BT_ALT_STACK
	else if (g_str_equal("Security", property)) {
		uint32_t mode;
		...
		return set_security(conn, msg, mode, data);
	}
#endif
				...
				struct btd_device *device, uint16_t handle)
{
	...
	info( "adapter_add_connection: adding device = %p handle = %d",  device, handle );
					...
					service_auth_cb cb, void *user_data)
{
	...
	struct agent *agent;
	char address[18];
	char value[PROPERTY_VALUE_MAX];	// Samsung SWP Bluetooth feature - IT Policy
	gboolean trusted;
	const gchar *dev_path;
	int err;
	...
	property_get("service.bt.security.policy.mode", value, "0");	// Samsung SWP Bluetooth feature - IT Policy

	if (trusted && !(strcmp(value, "1") == 0)) {
		cb(NULL, user_data);
		return 0;
	}
