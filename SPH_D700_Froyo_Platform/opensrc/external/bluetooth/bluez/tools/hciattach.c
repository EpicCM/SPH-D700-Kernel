...
static int bcm2035(int fd, struct uart_t *u, struct termios *ti)
{
	...
	case 2000000:
	case 2500000:
		/* Write UART Clock settng of 24MHz */
		cmd[4] = 0x02;
		break;
	case 3000000:
	case 4000000:
		/* Write UART Clock settng of 48MHz */
		cmd[4] = 0x01;
		break;
	default:
		/* Default is 921600 */
	...
	fprintf(stderr, "Uart clock parameters: clock=0x%2x\n",
		cmd[4]);

	/* Send command */
	if (write(fd, cmd, 5) != 5) {
		fprintf(stderr, "Failed to write \"set uart clock\" command\n");
		return -1;
	}

	if ((n = read_hci_event(fd, resp, 5)) < 0) {
		fprintf(stderr, "Failed to set uart clock\n");
		return -1;
	}

	/* Set the baud rate */
	/* use Explicit Baud rate */
	memset(cmd, 0, sizeof(cmd));
	memset(resp, 0, sizeof(resp));
	cmd[0] = HCI_COMMAND_PKT;
	cmd[1] = 0x18;
	...
	fprintf(stderr, "Baud rate parameters: {0x%2x,0x%2x,0x%2x,0x%2x}\n",
