...
// start brcm ryu 2010.06.01
static void obex_invalidate_parent_data(DBusConnection *conn, const char *child_path)
{
	struct generic_data *data = NULL;
	char *parent_path, *slash;
	...
	if (!dbus_connection_get_obex_object_path_data(conn, parent_path,
							(void *) &data))
		goto done;
...
static struct generic_data *obex_object_path_ref(DBusConnection *connection,
							const char *path)
{
	struct generic_data *data;

	if (dbus_connection_get_obex_object_path_data(connection, path,
						(void *) &data) == TRUE) {
		if (data != NULL) {
			data->refcount++;
			return data;
	...
	if (!dbus_connection_register_obex_object_path(connection, path,
						&generic_table, data)) {
		g_free(data->introspect);
		g_free(data);
		return NULL;
	...
	obex_invalidate_parent_data(connection, path);

	return data;
}

static void obex_object_path_unref(DBusConnection *connection, const char *path)
{
	struct generic_data *data = NULL;
	...
	dbus_connection_unregister_obex_object_path(connection, path);
}
// end brcm ryu
...
// start brcm ryu 2010.06.02
gboolean g_obex_dbus_register_interface(DBusConnection *connection,
					const char *path, const char *name,
					GDBusMethodTable *methods,
					GDBusSignalTable *signals,
					GDBusPropertyTable *properties,
					...
					GDBusDestroyFunction destroy)
{
	...
	data = obex_object_path_ref(connection, path);
	if (data == NULL)
		return FALSE;
...
gboolean g_obex_dbus_unregister_interface(DBusConnection *connection,
					const char *path, const char *name)
{
	struct generic_data *data = NULL;
	struct interface_data *iface;
	...
	obex_object_path_unref(connection, path);
