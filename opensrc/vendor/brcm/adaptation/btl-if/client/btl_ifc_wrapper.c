...
list_head ws_active_list;

#define DATAPATH_MAX_RX_SIZE 2048
char rx_data_buf[DATAPATH_MAX_RX_SIZE];
//#define verbose info
...
void wsactive_init(void)
{
    ...
    INIT_LIST_HEAD(&ws_active_list);  
}

void wsactive_add(t_wsock *ws,  int s)
{
    debug("add wsock %d to active list", ws->wsock);
    list_add(&ws->node, &ws_active_list);  
    FD_SET(s, &active_set);
    max_fd = MAX(s, max_fd);
}
...
BOOLEAN ws_in_active_list(t_wsock *ws)
{
    ...
    list_for_each(pos, &ws_active_list) 
    {
        ws_tmp = list_get_obj(pos, t_wsock, node);
        if (ws == ws_tmp)
            return TRUE;
    }
    return FALSE;
...
t_wsock* wrp_alloc_new_sock(int sub)
{
    ...
    for (i=0; i<SOCKET_USER_MAX;i++)
    {
        ...
        if (ws->busy == 0)
        {
            ...
            ws->async_msg_pnd = 0;  
            INIT_LIST_HEAD(&ws->node);
            INIT_LIST_HEAD(&ws->async_node);    
            return ws;
       }
    }
    return NULL;
...
{
   t_wsock *clone = wrp_alloc_new_sock(ws->sub);
   debug("sock %d cloned", ws->wsock);
   *clone = *ws;
   return clone;
}
...
int wrp_sock_bind(t_wsock* ws, int s, char *name, int port)
{
    ...
    verbose("s %d [%s], port %d", s, ws->name, port);
    ...
    debug("wrp_sock_bind:socket_local_server_bind result:%d server_name:%s\n", result, local_server_name);
...
#ifdef DTUN_LOCAL_SERVER_ADDR
    debug("local server name:%s, port:%d, fd:%d", ws->name, ws->port, s);
    do {
        len = sizeof(ws->address);
        sock_new = accept(s, &(ws->address), &len);
...
int wrp_sock_connect(t_wsock* ws, int s, char *name, int port)
{
    ...
    printf("wrp_sock_connect ret:%d server name:%s\n", result, local_server_name);
#else
...
BOOLEAN wrp_close_s_only(t_wsock *ws, int s)
{
    ...
    if ((ws->listen_fd == DATA_SOCKET_INVALID) && (ws->wsock == DATA_SOCKET_INVALID))
    {
        ...
        //ws_async_del( ws ); handled in blz wrapper
        wrp_wsock_init(ws);    
        return TRUE;
    }
...
void notify_data_ch_disc_ind(t_wsock *ws)
{
    ...
    btl_ifc_notify_local_event(ws->sub, BTLIF_DATA_CHAN_DISC_IND, &param);    
}

static int check_data_sock(t_wsock *ws)
{
...
int wrp_check_fdset(t_wsock *ws)
{
    if ((ws->listen_fd !=DATA_SOCKET_INVALID) && (FD_ISSET(ws->listen_fd, &read_set)))
    {
        tDATA_HANDLE hdl;
...
int wrp_check_active_handles(void)
{
    ...
    list_for_each(pos, &ws_active_list) 
    {
        ws = list_get_obj(pos, t_wsock, node);
        
        fds_processed += wrp_check_fdset(ws);
    }
