...
int obex_dbus_init (DBusConnection *in_conn)
{
    ...
    conn = dbus_obex_connection_ref( in_conn );
    // end brcm ryu
    debug( "Obex: g_dbus_register_interface  in_conn = %p conn=%p", in_conn, conn );
    // start brcm ryu 2010.06.02
    if (!g_obex_dbus_register_interface(in_conn, BTLA_OBEX_PATH, BTLA_OBEX_INTERFACE,
...
void obex_dbus_exit (void)
{
    ...
    // start brcm ryu 2010.06.02
    if (!conn || !dbus_connection_get_obex_is_connected(conn)) {
        return;
    }
    g_obex_dbus_unregister_interface(conn, BTLA_OBEX_PATH, BTLA_OBEX_INTERFACE);
    // end brcm ryu
