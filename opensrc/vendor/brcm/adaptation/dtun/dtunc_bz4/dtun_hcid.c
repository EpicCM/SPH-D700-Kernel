...
#define DTUN_PROPERTY_AUDIO_A2DP_CONNECT  "audio.a2dp.connect"
...
void dtun_init_device_uuid( struct btd_device *device, char *str_uuid )
{
    ...
    if (strcmp( str_uuid, A2DP_SINK_UUID ) == 0 )
    {
        ...
        if (audio_dev != NULL) {
            ///Sandeep:setting property to system property to "0" :audio.a2dp.connect
            //sandeep:start
            if(property_set(DTUN_PROPERTY_AUDIO_A2DP_CONNECT,"0")<0)
                info("sandeep:Failed to set property 0 ");
            else
                info("sandeep:A2DP stream :0");
            //sandeep:end
...
int dtunops_send_set_tx_carrier_frequency (int carrier, int frequency, int mode,
                                           int modulation, int txpower, int txpower_dbm, int txpower_index)
{
        tDTUN_DEVICE_METHOD method;

  //      method.set_security.hdr.id  = DTUN_METHOD_DM_SEND_SET_TX_CARRIER_FREQUENCY;
  //      method.set_security.hdr.len = sizeof(tDTUN_METHOD_DM_SEND_SET_TX_CARRIER_FREQUENCY) - sizeof(tDTUN_HDR);

        method.send_set_tx_carrier_frequency.carrier       = carrier;
        method.send_set_tx_carrier_frequency.frequency     = frequency;
        method.send_set_tx_carrier_frequency.mode          = mode;
        method.send_set_tx_carrier_frequency.modulation    = modulation;
        method.send_set_tx_carrier_frequency.txpower       = txpower;
        method.send_set_tx_carrier_frequency.txpower_dbm   = txpower_dbm;
        method.send_set_tx_carrier_frequency.txpower_index = txpower_index;
...
static struct btd_adapter_ops dtun_ops = {
    ...
    .send_set_tx_carrier_frequency = dtunops_send_set_tx_carrier_frequency,
};
...
static void dtun_am_sig_av_event(tDTUN_DEVICE_SIGNAL *msg)
{
	...
	if( pdev )
	{
		...
		///Sandeep:setting property to system property to "1" :audio.a2dp.connect
		//sandeep:start
		char value[PROPERTY_VALUE_MAX];
		property_get(DTUN_PROPERTY_AUDIO_A2DP_CONNECT,value,NULL);
		if(!strcmp(value,"0"))
		property_set(DTUN_PROPERTY_AUDIO_A2DP_CONNECT,"1");
		//sandeep:end	
            g_dbus_emit_signal(pdev->conn, pdev->path, AUDIO_SINK_INTERFACE,
